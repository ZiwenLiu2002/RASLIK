...
import torch
from RASLIK.calc_inner import grad_z, calc_loss, get_params, normalize, pad, reshape
from RASLIK.data_loader import get_model_tokenizer, TrainDataset, TestDataset, get_tokenizer, get_model
from RASLIK.data_loader import get_dataset_size, read_data
from RASLIK.influence_function import calc_s_test_single
from RASLIK.utils import save_json, display_progress, load_json
from RASLIK.RapidGrad import RapidGrad
import numpy as np
import time
import json
...
def MP_run_calc_infulence_function(rank, world_size, process_id, config, mp_engine, restart = False):
    ...
    with mp_engine.gpu_locks[rank].get_lock():
        ...
        with mp_engine.gpu_locks_num.get_lock():
            display_progress("Model Loading: ", mp_engine.gpu_locks_num.value, config.influence.n_threads*world_size, cur_time=time.time())
            ...
            if cal_word_infl < 0:
                ...
                if grad_z_vec is None:
                    ...
                    if config.influence.RapidGrad.enable and isinstance(config.influence.RapidGrad.RapidGrad_K, list):
                        ...
                        for i, (path, k) in enumerate(zip(config.influence.RapidGrad.multi_k_save_path_list, config.influence.RapidGrad.RapidGrad_K)):
    ...
    while True:
        ...
        if not config.influence.skip_influence and ((i + 1) % (total_size // 50) == 0 or i == total_size - 1):
            for j in range(test_dataset_size):
                ...
                # new：only generate the whole helpful list
                sorted_ids = np.argsort(infl_list[j]).tolist()
                helpful = [
                    shuffled_id2real_id[x]
                    for x in sorted_ids[::-1]
                    if x in shuffled_id2real_id
                ]
                raw = infl_list[j]
                python_infl = [
        ...
        for j in range(test_dataset_size):
            ...
            with mp_engine.train_idx.get_lock(), mp_engine.finished_idx.get_lock(), mp_engine.cal_word_infl.get_lock():
                for x in influences[j]['helpful']:
                    idx = real_id2shuffled_id[x]
                    mp_engine.cal_word_infl[idx] = j
                    mp_engine.finished_idx[idx]   = False
